...
# ╔═╡ 4c3c64dd-db38-49a4-9360-840a8d9b5792
module CategoryWiring
	
	using Catlab.WiringDiagrams
	using Catlab.GraphvizGraphs
	
	# Macro for defining objects (nodes)
	macro object(wd, name)
	    quote
	        # Add the object (box) to the wiring diagram, store it in a local variable
	        local $name = add_box!($wd, Box(Symbol($name), [:input], [:output]))
	    end
	end
	
	# Macro for defining morphisms (arrows)
	macro morphism(wd, from_obj, to_obj)
	    quote
	        # Add a wire (morphism) between two objects (boxes)
	        add_wire!($wd, ($from_obj, 1) => ($to_obj, 1))
	    end
	end
	
	# Macro to create the wiring diagram (category)
	macro category(objects, morphisms)
	    quote
	        # Create the wiring diagram
	        wd = WiringDiagram([:outer_input], [:outer_output])
	
	        # Ensure objects and morphisms are collections
	        local objects_list = $objects
	        local morphisms_list = $morphisms
	
	        # Define the objects (nodes)
	        for obj in objects_list
	            @object wd obj
	        end
	
	        # Define the morphisms (arrows)
	        for morph in morphisms_list
	            @morphism wd morph[1] morph[2]
	        end
	...
	# A helper function to visualize the category (wiring diagram)
	function visualize_category(wd::WiringDiagram)
	    graph = to_graphviz(wd)
	    svg = sprint(IOBuffer) do io
	        write(io, graph)
	    end
	    return svg
	end
	
	export @object, @morphism, @category, visualize_category
...
manifest_format = "2.0"
project_hash = "cdc6961bb2c1b1f6e6f7afeb94d748c541cad5c9"
...
git-tree-sha1 = "d071b7329e50bd4dbb7e3893d3bc209cc27d22ad"
...
git-tree-sha1 = "bfa80c8fc846dde1b7c06cc8ef8434b124ce4e1f"
...
git-tree-sha1 = "6151c0db203db299cec0090f78910be5efc50fb9"
